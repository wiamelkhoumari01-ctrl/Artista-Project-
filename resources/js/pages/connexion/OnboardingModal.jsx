...
export default function OnboardingModal({ onClose }) {
  ...
  // ── Récupère le token du NOUVEL utilisateur depuis le localStorage ──
  // Ce token a été stocké par handleAuthSuccess juste avant l'affichage du modal
  const getToken = () => localStorage.getItem('access_token');

  // ── Instance axios avec le bon token — jamais le token de l'admin ──
  ...
  React.useEffect(() => {
    ...
    if (!token) return;
    authApi(token).get('/api/categories')
  ...
  const handleStep1Submit = async () => {
    if (!form.stage_name || !form.city || !form.country) {
      setError('Veuillez remplir les champs obligatoires (nom de scène, ville, pays).');
      return;
    }
    setError('');
    setLoading(true);
    ...
    try {
      await authApi(token).post('/api/artist/update', {
      ...
      const msg = e.response?.data?.message || e.response?.data?.error || 'Erreur lors de la sauvegarde.';
      setError(msg);
      console.error('[Onboarding Step1]', e.response?.data);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStep2Submit = async () => {
    ...
    if (photoFile && token) {
      ...
      try {
        await axios.post('http://127.0.0.1:8000/api/artist/upload-photo', fd, {
          headers: {
            'Authorization': `Bearer ${token}`,
        ...
        console.warn('[Onboarding Step2] Upload photo échoué:', e.response?.data);
      }
    }
...
const styles = {
  ...
  progressBg: { height: 4, background: '#f0e8dc', margin: '20px 32px 0', borderRadius: 99 },
  progressFill: {
    height: '100%', background: 'linear-gradient(90deg, #c5a059, #e8c97a)',
    borderRadius: 99, transition: 'width 0.5s ease',
  },
  body:     { padding: '28px 32px 36px' },
  title:    { fontFamily: 'Playfair Display, serif', fontSize: 26, fontWeight: 700, color: '#1a1a1a', marginBottom: 8 },
  ...
  btnDisabled: {
    ...
    cursor: 'not-allowed', marginTop: 8, fontFamily: 'inherit',
  },
  btnOutline: {
    display: 'block', margin: '12px auto 0',
    padding: '8px 20px', background: 'transparent',
    border: '1.5px solid #c5a059', borderRadius: 99,
    color: '#c5a059', fontWeight: 600, fontSize: 13,
  ...
  photoPreview: {
    ...
    objectFit: 'cover', border: '4px solid #c5a059',
  },
  langGrid: { display: 'flex', gap: 16, marginBottom: 32, flexWrap: 'wrap' },
  langCard: {
    flex: 1, minWidth: 130,
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10,
    padding: '24px 16px', border: '2px solid', borderRadius: 18,
    cursor: 'pointer', transition: 'all 0.2s',
    background: '#fff', position: 'relative', fontFamily: 'inherit',
  },
  langLabel: { fontSize: 15, fontWeight: 600, color: '#333' },
  langCheck: { position: 'absolute', top: 10, right: 14, color: '#c5a059', fontWeight: 700, fontSize: 16 },
};
